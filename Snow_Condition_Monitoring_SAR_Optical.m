...
close all;
%%
file = double(imread('Path to File'));
clc;
%% Optical
clear optical1
optical1 = file(:,:,8:13);
clc;
%% Snow Contamination index, Band-1 is Blue, Band-2 is Green
clear SCI
SCI = (optical1(:,:,1)-optical1(:,:,2))./(optical1(:,:,1)+optical1(:,:,2));
SCI_res = reshape(SCI,[],1);

%% Normalized difference snow index, Band-2 is Green, Band-5 is SWIR-1
clear NDSI NDIS
...
%%
clear SD_field
SD_field = [20;40;33;31;25]; %%% Snow Depth in cm observed in the field
NDFSI = (optical1(:,:,4)-optical1(:,:,5))./(optical1(:,:,4)+optical1(:,:,5));
NDVI = (optical1(:,:,4)-optical1(:,:,3))./(optical1(:,:,4)+optical1(:,:,3));
figure
imshow(NDFSI)
title('Normalized Difference Forest Snow Index')
%%
...
figure
imshow(reshape(S3_res,size(S3,1),size(S3,2)),[])
title('S3 Snow')
...
ylabel('NIR')
clc;
%% Wetness estimation of snowpack
clear w W
id = 0.231127;
iw = 0.00594186;
sd = (0.893712-id);
...
figure
subplot(1,6,1)
boxplot([((Radar_conta(:,1))),((Radar_clear(:,1)))],'Labels',{'Contaminated','Clear'})
ylabel('Backscattering in dB VH')
subplot(1,6,2)
boxplot([((Radar_conta(:,2))),((Radar_clear(:,2)))],'Labels',{'Contaminated','Clear'})
ylabel('Backscattering in dB VV')
subplot(1,6,3)
boxplot([((Radar_conta(:,4))),((Radar_clear(:,4)))],'Labels',{'Contaminated','Clear'})
ylabel('LIA')
subplot(1,6,4)
...
figure
subplot(1,2,1)
scatter((Radar_conta(:,4)),(Radar_conta(:,1)))
xlabel('LIA in degrees')
xlim([20 60])
ylabel('VH (dB)')
subplot(1,2,2)
scatter((Radar_conta(:,4)),(Radar_conta(:,2)))
xlabel('LIA in degrees')
xlim([20 60])
ylabel('VV (dB)')
clc;
...
title('RGB')
clc;
%% Overlay of clear and contaminated snow on RGB Image
